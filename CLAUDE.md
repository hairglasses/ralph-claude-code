...
### Exit Summary Report
On any exit (graceful completion, budget/runtime limit, circuit breaker, permission denied, integrity failure, or API limit), Ralph displays a formatted summary showing:
- Exit reason
- Total loops completed
- API calls used
- Estimated cost (USD)
- Runtime duration
- Files changed (git diff from run start SHA to current HEAD)

The summary is generated by `generate_exit_summary()` and outputs both a visual box to stdout and structured log entries.
